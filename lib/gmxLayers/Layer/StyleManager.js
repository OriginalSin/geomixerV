...
/*eslint-enable */
    ...
    _parseStyle: function(st, renderStyle) {
        if (st) {
			...
			st.webgl = {
 				fillStyle: gmxAPIutils.dec2rgbWebGL(st.fillColor, st.fillOpacity),
				strokeStyle: gmxAPIutils.dec2rgbWebGL(st.color, st.opacity)
			};
        }
        return st;
    },
...
/*eslint-enable */
    ...
    _itemStyleParser: function(item, pt) {
        ...
        if (type === 'image') {
			...
			webgl.strokeStyle = gmxAPIutils.dec2rgbWebGL(color, opacity);
        ...
        if (type === 'square' || type === 'polygon' || type === 'circle' || type === 'iconPath') {
            ...
            if ('fillColor' in pt) {
                out.fillStyle = gmxAPIutils.dec2color(fcDec, 1);
 				webgl.fillStyle = gmxAPIutils.dec2rgbWebGL(fcDec, 1);
           }
            if ('fillColorFunction' in pt || 'fillOpacityFunction' in pt) {
				...
				webgl.fillStyle = gmxAPIutils.dec2rgbWebGL(fcDec, fop);
            }
        }
