...
*/
    ...
    redrawItem: function (item) {
        if (this._map) {
				let bounds = item.allBouns;
				if (!bounds && typeof(item) !== 'object') { bounds = this._gmx.dataManager.getItemBounds(item); }
				var gmxTiles = this._getTilesByBounds(bounds);
				this.repaint(gmxTiles);
			// if (typeof(item) !== 'object') { item = this._gmx.dataManager.getItem(item); }
			// if (item) { // Todo: выбор тайлов по id
    ...
    _updateProperties: function (prop) {
        ...
        if ('MetaProperties' in gmx.rawProperties) {
            ...
            if ('webGL' in meta) {        		// Слой через WebGL
                gmx.webGL = meta.webGL.Value === 'true';
            }
        }
