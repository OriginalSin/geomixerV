...
var gmxAPIutils = {
    ...
    */
    ...
    dec2rgbWebGL: function(i, a)	{				// convert decimal to rgb
        var r = (i >> 16) & 255,
            g = (i >> 8) & 255,
            b = i & 255;
    ...
    setSVGIcon: function(id) {
		return '<svg role="img" class="svgIcon ' + id + '"><use xlink:href="#' + id + '" href="#' + id + '"></use></svg>';
    },
...
*/
    ...
    */
    getLength: function(latlngs, isMerc, isWebMerc) {
        let length = 0;
        if (latlngs && latlngs.length) {
            let prev = false;

            isWebMerc = isWebMerc === undefined || isWebMerc;
            isMerc = isMerc === undefined || isMerc;
            latlngs.forEach(function(latlng) {
                if (Array.isArray(latlng)) {
                    if (Array.isArray(latlng[0])) {
                        length += gmxAPIutils.getLength(latlng, isMerc, isWebMerc);
                        return length;
                    } else if (isWebMerc) {   // From WebMercator array
                       latlng = L.CRS.EPSG3857.unproject({x: latlng[0], y: latlng[1]});
                    } else if (isMerc) {   // From Mercator array
                        latlng = L.Projection.Mercator.unproject({x: latlng[0], y: latlng[1]});
                    }
                }
                if (prev) length += prev.distanceTo(latlng);
				prev = latlng;
            });
        }
        return length;
    ...
    */
    geoJSONGetLength: function(geoJSON) {
        ...
        if (geoJSON.type === 'GeometryCollection') {
			...
			out += coords.reduce((a, c) => { return a + gmxAPIutils.getRingLength(c); }, out);
        } if (geoJSON.type === 'Polygon' || geoJSON.type === 'MultiPolygon') {
            coords = geoJSON.coordinates;
            if (geoJSON.type === 'Polygon') coords = [coords];
			out += coords.reduce((a, c) => { return c.reduce((a1, c1) => { return a1 + gmxAPIutils.getRingLength(c1); }, a); }, out);
        }
        return out;
    },
    ...
    getRingLength: function(coords) {
        ...
        if (coords && coords.length) {
            ...
            coords.forEach(function(lnglat) {
				...
				let lnglatObj = L.latLng(lnglat[1], lnglat[0]);
                if (prev) length += prev.distanceTo(lnglatObj);
				prev = lnglatObj;
            });
        }
        return length;
    ...
    */
    geoJSONGetArea: function(geoJSON) {
        ...
        if (geoJSON.type === 'GeometryCollection') {
			...
			out += coords.reduce((a, c) => {
                return a + gmxAPIutils.getRingArea(c[0]) - c.slice(1).reduce((a1, c1) => { return a1 + gmxAPIutils.getRingArea(c1); }, a);
			}, out);
        }
        return out;
    },
    ...
    getRingArea: function(coords) {
        ...
        if (!len) return 0;
        let area = coords.reduce((a, c, i) => {
            let ipp = (i === (len - 1) ? 0 : i + 1), p1 = coords[i], p2 = coords[ipp];
            a += p1[0] * Math.sin(gmxAPIutils.degRad(p2[1])) - p2[0] * Math.sin(gmxAPIutils.degRad(p1[1]));
			return a;
		}, 0);
		return area ? Math.abs(area * gmxAPIutils.lambertCoefX * gmxAPIutils.lambertCoefY / 2) : 0;
    },
...
L.extend(L.gmxUtil, {
	...
	dec2rgbWebGL: gmxAPIutils.dec2rgbWebGL,
