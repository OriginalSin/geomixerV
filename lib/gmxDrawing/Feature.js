    ...
    _initialize: function (parent, obj) {
        ...
        if (this.options.editable) {
			...
			if (Utils.isOldVersion) {
				...
				} else if (this.options.type === 'MultiPolyline') {
					arr = (obj.getLayers ? obj.getLayers()[0] : obj)
						.getLatLngs()
						.map(function(it) { return {_latlngs: it}; });
				} else if (this.options.type === 'Polyline') {
					var _latlngs = (obj.getLayers ? obj.getLayers()[0] : obj).getLatLngs();
					arr = [{_latlngs: _latlngs}];
				}
			}
            for (var i = 0, len = arr.length; i < len; i++) {
                ...
                if (it._holes) {
                    for (var j = 0, len1 = it._holes.length; j < len1; j++) {
                        var hole = new Ring(this, it._holes[j], {hole: true, forRing: ring, editable: this.options.editable});
                        this.addLayer(hole);
                        holes.push(hole);
                    }
                }
