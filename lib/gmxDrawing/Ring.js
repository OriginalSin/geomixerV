...
const Ring = L.LayerGroup.extend({
    ...
    _createPointsNum: function () {
		...
		const begShift = opt.shiftBegin || 0;
		const endShift = opt.shiftEnd || 0;
		const dx = 6, dy = -10;
		const _createItem = (it, nm, cont, count) => {
			let b = document.createElementNS(svgNS, 'rect');
			let shiftB = nm === 1 ? begShift : 0;
			let shiftE = nm === count ? endShift : 0;
			b.setAttributeNS(null, "x", it.x - 4 + dx + shiftB - shiftE);
			...
			t.setAttributeNS(null, "x", it.x + dx + shiftB - shiftE);
			t.setAttributeNS(null, "y", it.y + dy);
			if (opt.startNum) nm += opt.startNum;
		...
		this.showTooltipPoints = function(textArr) {
			if (textArr && textArr.length) {
				...
				textArr.forEach((it, i) => {
					let nm = i + 1;
					if (opt.skipBegin && !i) return;
					if (opt.skipLast && nm === textArr.length) return;
					_createItem(it, nm, this.tPoints, textArr.length);
				});
			}
		};
 	},
    ...
    _eventsCmd: function (obj, ev) {
		...
		this._fireEvent('change', downAttr);
    },
    ...
    _mouseupPoint: function (ev) {
        ...
        if (ctrlKey && this.down && Utils.chkGeoJson(this.points.toGeoJSON()).length) {
			latlng = this._lastSnapLatlng;
			_setPoint = true;
		}
       if (_setPoint && this.down) {
		...
		this._fireEvent('mouseup', ev);
		this._fireEvent('change', ev);
    },

    _pointMove: function (ev) {
		ev.target._opt = 'pMove';
		const ctrlKey = ev.originalEvent.ctrlKey;
        ...
        if (this.down && this._lastDownTime < Date.now()) {
            ...
            this._moved = true;
			if (!this._chkHolePoint(ev.layerPoint)) return;
